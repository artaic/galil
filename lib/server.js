...
Galil._createConnection = function(name) {
  ...
  socket.connect(Meteor.settings.galil.connection);
...
function MessageListener(socket, listenFunction, timeout=Galil.config.defaultTimeout) {
  ...
  let callback = listenFunction.bind(this, future.resolver());
...
Galil.execute = function(subroutine, onData=_.noop) {
  ...
  this.connections.update({}, { $set: { executing: true } }, { multi: true });
  ...
  let future = new MessageListener(this._messages, (done, data) => {
    ...
    if (success.test(data.toString('ascii'))) {
      this.connections.update({}, { $set: { executing: false } }, { multi: true });
