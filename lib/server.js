...
Galil._createConnection = function(name) {
  ...
  let socket = net.connect(Number(process.env.GALIL_PORT), process.env.GALIL_HOST, Meteor.bindEnvironment(() => {
...
Galil.execute = function(subroutine, timeout=this.config.defaultTimeout) {
  ...
  let timeoutId = Meteor.setTimeout(() => {
    _.each(listeners, (listener) => this.addListener(listener));
    onComplete(new Meteor.Error('GalilTimeout', `Execution of ${subroutine} timed out after ${timeout} milliseconds`));
  }, timeout);

  this.addListener('End', Meteor.bindEnvironment(() => {
    _.each(listeners, (listener) => this.addListener(listener));
  ...
  this.addListener('Error', Meteor.bindEnvironment(() => {
    _.each(listeners, (listener) => this.addListener(listener));
    Meteor.clearTimeout(timeoutId);
    onComplete(new Meteor.Error('GalilError', `Failed execution of subroutine ${subroutine}`), null);
  }));
